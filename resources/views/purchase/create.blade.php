...
@section('purchase')
                    ...
                    <div class="row ms-3 me-3">
                        <div class="col-lg-6 col-md-6">
                            ...
                            <div class="mb-3 px-3 pt-3">
                                ...
                                <select class="form-control" id="id_distributor" name="id_distributor">
                                    ...
                                    @foreach($distributors as $distributor)
                                        <option value="{{ $distributor->id }}"@if (isset(session('data')->id_distributor) && session('data')->id_distributor == $distributor->id) selected @endif>
                            ...
                            <div class="mb-3 px-3 pt-3">
                                ...
                                <select class="form-control" id="id_barang" name="id_barang">
                                    ...
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}"@if (isset(session('data')->id_barang) && session('data')->id_barang == $product->id) selected @endif>
                                        {{ $product->nama_barang }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3 px-3 pt-3">
                                ...
                                <input type="text" class="form-control" id="harga_beli" name="harga_beli" placeholder="Enter Product Purchase Price" value="@if (isset(session('data')->harga_beli))
                                    {{ session('data')->harga_beli }}
                                @endif">
                            </div>
                            <div class="mb-3 px-3 pt-3">
                                <label for="margin_jual" class="form-label">Selling Margin</label>
                                <input type="text" class="form-control" id="margin_jual" name="margin_jual" placeholder="Enter Product Selling Margin" value="@if (isset(session('data')->margin_jual))
                                    {{ session('data')->margin_jual }}
                                @endif">
                            </div>
                        </div>
                        <div class="col-lg-6 col-md-6">
                            ...
                            <div class="mb-3 px-3 pt-3">
                                ...
                                <input type="text" class="form-control" id="harga_jual" name="harga_jual" placeholder="Enter Product Selling Price" value="@if (isset(session('data')->harga_jual))
                                    {{ session('data')->harga_jual }}
                                @endif" readonly>
                            </div>
                            <div class="mb-3 px-3 pt-3">
                                <label for="jumlah_beli" class="form-label">Purchase Amount</label>
                                <input type="text" class="form-control" id="jumlah_beli" name="jumlah_beli" placeholder="Enter Product Purchase Amount" value="@if (isset(session('data')->jumlah_beli))
                                    {{ session('data')->jumlah_beli }}
                                @endif">
                            </div>
                            <div class="mb-3 px-3 pt-3">
    ...
    let distributor = document.getElementById('id_distributor');
    ...
    let total_bayar_input = document.getElementById('total_bayar'); // Nama variabel dibedakan dari fungsi
    ...
    // Fungsi hitung total bayar yang diperbaiki
    ...
    harga_beli.addEventListener('keyup', function() {
        let hb = parseInt(harga_beli.value) || 0;
        let mj = parseInt(margin_jual.value) || 0;
        let jb = parseInt(jumlah_beli.value) || 0;

        harga_jual.value = hargaJual(hb, mj);
        subtotal.value = subTotal(hb, jb);
        jalankanHitungTotal(); // Panggil fungsi hitung
    });
    ...
    btnSimpan.addEventListener('click', function() {
        // Validasi
        if(no_nota.value.trim() === '' || distributor.value === '' || id_barang.value === '' || tgl_nota.value === '') {
            swal("Invalid!", "Please fill all required fields!", "error");
            return;
        }
        
        if(parseInt(harga_beli.value) <= 0 || parseInt(jumlah_beli.value) <= 0) {
            swal("Invalid!", "Price and Amount must be more than 0!", "error");
            return;
        }

        // Jika semua OK, submit form secara manual
        form.submit();
    });
